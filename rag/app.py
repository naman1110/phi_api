...
def fetch_pdf_from_url():
    """Fetch a PDF file from the given URL."""
    try:
...
def main() -> None:
    ...
    embeddings_model = st.sidebar.selectbox(
        "Select Embeddings",
        options=["text-embedding-3-large","nomic-embed-text","text-embedding-3-small"],
    ...
    try:
        ...
        print("Run ID:", st.session_state["rag_assistant_run_id"])
    except Exception:
    ...
    if rag_assistant.knowledge_base:
        ...
        st.sidebar.title("Knowledge Base Operations")

        # PDF fetching from API
        pdf_url = st.sidebar.text_input("Enter PDF URL:", "")
        fetch_pdf_button = st.sidebar.button("Fetch PDF from URL")

        if fetch_pdf_button and pdf_url:
            st.sidebar.info("Fetching PDF...")
            uploaded_file = fetch_pdf_from_url(pdf_url)

            if uploaded_file:
                alert = st.sidebar.info("Processing PDF...")
                pdf_file = io.BytesIO(uploaded_file)
                reader = PDFReader(chunk_size=3200)
                rag_documents = reader.read(pdf_file)
                if rag_documents:
